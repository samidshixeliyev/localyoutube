-- ═══════════════════════════════════════════════════════════════════
-- ModTube — single initial schema (all migrations consolidated)
-- ═══════════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────
-- Videos  (is_short included from the start)
-- ─────────────────────────────────────────────
...
('idp.base-url',            'https://13.61.159.58',                'IDP base URL',                                   NOW()),
...
('idp.claim.email',         'mail',                                'JWT claim for user email',                       NOW()),
('idp.claim.fullname',      'cn',                                  'JWT claim for full display name',                NOW()),
('idp.claim.first',         'givenName',                           'JWT claim for given name',                       NOW()),
('idp.claim.last',          'sn',                                  'JWT claim for surname',                          NOW()),
('idp.claim.username',      'uid',                                 'JWT claim for login username',                   NOW()),
('upload.max-parallel',     '2',                                   'Max parallel chunk uploads per video (1-10)',    NOW()),
('grafana.url',             '',                                    'Grafana URL (empty = auto-detect as host:3000)', NOW())
